...
- Added `ReadOnlyMiddleware.wrap()` and `WriteOnlyMiddleware.wrap()` to help with
  type inference.

### Fixed

- Fixed typing of `SupervisionFunction`, which is supposed to be fully compatible with
  the signature of the `supervise()` function. The `name` argument added earlier in
  9.2.0 was not reflected in `SupervisionFunction` until now.
